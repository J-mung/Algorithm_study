...
public class Base64Decoder_1928 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < tc; i++) {
            ...
            // 표를 보고 입력받은 문자들을 각각 대응하는 숫자로 변경
            for (int j = 0; j < encoding.length(); j++) {
                int decimal = 0;

                char encoChar = encoding.charAt(j);
                if (encoChar >= 'A' && encoChar <= 'Z')
                    decimal = encoChar - 'A';
                else if (encoChar >= 'a' && encoChar <= 'z')
                    decimal = encoChar - 'a' + 26;
                else if (encoChar >= '0' && encoChar <= '9')
                    decimal = encoChar - '0' + 52;
                else if (encoChar == '+')
                    decimal = 62;
                else
                    decimal = 63;

                // 각 숫자들을 6자리 이진수로 표현하고
                String binaryTmp = Integer.toBinaryString(decimal);

                // 만약 6자리 보다 짧으면 보충
                for (int k = 6 - binaryTmp.length(); k > 0; k--) {
                    binaryTmp = "0" + binaryTmp;
                }

                // 이 이진수들을 한 줄로 이어 붙임
                binaryString += binaryTmp;
            }

            for (int j = 0; j < binaryString.length(); j += 8) {

                // 한 줄로 이어진 이진수들을 8자리씩 끊어서
                String binaryTmp = binaryString.substring(j, j + 8);

                // 십진수로 바꿈
                int binaryToDecimal = Integer.parseInt(binaryTmp, 2);

                // 각각의 십진수를 아스키 코드로 변환
                decoding.append((char) binaryToDecimal);
            }
